...
<body class="bg-white">
    <div class="container mx-auto p-4">
        <h1 class="text-3xl font-bold mb-4 text-black">Tambah CCTV</h1>
        @if ($errors->any())
        ...
        @endif
        <form action="{{ route('buildings.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-lg border border-gray-200">
            @csrf
            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-black">Nama Gedung:</label>
                <input type="text" id="name" name="name" required class="input input-bordered w-full bg-white" value="{{ old('name') }}">
            </div>
            <div class="mb-4">
                <label for="floor" class="block text-sm font-medium text-black">Lantai:</label>
                <input type="text" id="floor" name="floor" required class="input input-bordered w-full bg-white" value="{{ old('floor') }}">
            </div>
            <div class="mb-4">
            ...
            <button type="submit" class="btn bg-red-500 hover:bg-red-600 text-white border-none w-full">Tambahkan</button>
        </form>
    </div>
</body>
</html>
